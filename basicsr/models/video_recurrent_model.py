...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def test(self):
        ...
        double_flip_seq = self.opt['val'].get('double_flip_seq', False)
        ...
        if flip_seq:
            ...
            output_2 = self.output[:, n:2*n, :, :, :].flip(1)
            if not double_flip_seq:
                self.output = 0.5 * (output_1 + output_2)
            else:
                output_3 = self.output[:, 2*n:3*n, :, :, :]
                output_4 = self.output[:, 3*n:4*n, :, :, :].flip(1)
                self.output = 0.25 * (output_1 + output_2 + output_3 + output_4)
