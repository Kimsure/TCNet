...
def forward_crop(x, model, lq_size=64, scale=4, overlap=16, flow_opt=False):
    ...
    assert lq_size == 64 or 48, "Default patch size of LR images during training and validation should be {}.".format(
        lq_size)
        ...
        for t in range(T):
            ...
            for i in range(I):
                ...
                for j in range(J):
                    ...
                    B_end = B_start + B
                    # print(B_start, B_end)
                    x[:, t, :, i_start: i_end, j_start: j_end] = x_crops[B_start: B_end, t, :, pi_start: pi_end,
                                                                 pj_start: pj_end]
        print(x.shape)
